	...
	<main class="flex flex-1 items-center justify-center px-4 pb-20 pt-14 sm:px-6 sm:pt-16">
		<div class="grid w-full max-w-6xl items-start gap-12 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)] lg:gap-16">
			<section class="flex flex-col gap-8">
				...
				<div class="grid gap-4 rounded-3xl bg-neutral-50/80 p-6 text-sm text-neutral-600 sm:grid-cols-3">
					{#each about.stats as stat}
						<div>
							<p class="font-lifted text-[0.65rem] uppercase tracking-[0.45em] text-neutral-400">{stat.label}</p>
				...
				<div class="space-y-5">
					<p class="text-xs uppercase tracking-[0.35em] text-neutral-500">Projecten waar ik trots op ben</p>
					<div class="space-y-6">
						{#each about.projects as project}
							<a
								...
								target="_blank"
								rel="noreferrer"
								class="group flex flex-col gap-2 border-b border-neutral-200 pb-5 transition-colors hover:border-rose-600"
							>
								<span class="font-display text-xl text-neutral-900 transition-colors group-hover:text-rose-700">{project.title}</span>
								<span class="text-sm text-neutral-500">
									{project.description}
								</span>
								<span
									class="font-lifted text-[0.65rem] uppercase tracking-[0.4em] text-neutral-400 transition-all group-hover:translate-x-1 group-hover:text-rose-700"
									>Bekijk project →</span
								>
							</a>
						{/each}
			...
			<section class="flex flex-col gap-8">
				...
				<div class="grid gap-6 rounded-3xl bg-rose-700/95 p-8 text-rose-50">
					...
					<p class="font-lifted text-[0.65rem] uppercase tracking-[0.45em] text-rose-200">
